...
class HumanoidBulletEnv(gym.Env):
    ...
    def step(self, ctrl):
        ...
        #print("feet {:.4f} pelvis {:.4f} neck {:.4f}".format(feet,pelvis,neck))
        #(np.square(xd) + np.square(yd) + np.square(zd))*.2
        r_neg = neck*2 + pelvis*3 + feet
        r_pos = (1-torso_good_rot)*2 + (2 - legs_rot) + (1 - (abs(roll) + abs(pitch))/2)*3
        r = np.clip(r_pos - r_neg, -5, 5)
        #print("pos {}, neg {}, r {}".format(r_pos,r_neg, r))
        #print("feet {}, pelvis {}, neck {}".format(feet,pelvis,neck))
        #print("tor {}, legs {}".format(max(1-.4-torso_good_rot, 0), max(1-.4 - legs_rot/2, 0)*2, (1 - (abs(roll) + abs(pitch))/2)))
        """reward
        """
